...
class PurchaseController extends Controller
{
    public function index() {
        $purchaseInvoice = PurchaseInvoice::latest()->first();
        $purchaseClients = PurchaseClientMaster::orderBy('name','ASC')->get();
        $depots = DepotMaster::orderBy('depotName','ASC')->get();
        $groups = GroupMaster::where('id','<=','2')->get();
        $ledgers = LedgerMaster::where('id','<=','1')->get();
        return view('transaction.trading.purchase', compact('purchaseInvoice','purchaseClients','depots','groups','ledgers'));
    }
    ...
    public function getItemDetail(Request $request){
        $itemCode = $request->itemCode;
        if(!empty($itemCode)){
            $itemData = ItemMaster::where('code','LIKE','%'.$itemCode.'%')->with('tax')->get();
            return response()->json([
                'status'=>true,
                'data'=>$itemData
            ]);
        ...
        try {
            ...
            foreach ($items as $key => $item) {
                ...
                $itemStock = ItemStock::updateOrCreate(
                    [
                        'referenceNo' => $purchaseItem->id
                    ],
                    [
                        'itemCode' => $item['code'],
                        'itemName' => $item['name'],
                        'itemUnit' => $item['unit'],
                        'purchaseDate' => $request->invoiceDate,
