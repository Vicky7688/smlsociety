    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card page_headings cards mb-4">
            <div class="card-body py-2">
                <div class="row">
                    <div class="col-md-6">
                        <h4 class="py-2"><span class="text-muted fw-light">Reports / </span>Contribution List</h4>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <form action="javascript:void(0)" id="formData" name="formData">
                            <div class="row">
                                ...
                                <div class="col-lg-2 col-md-3 col-sm-4 col-6 py-3 inputesPaddingReport">
                                    ...
                                    <input type="text" name="memberType"class="form-control formInputsReport"
                                        id="memberType" value="Member" readonly>
                                </div>
                                 <div class="col-lg-2 col-md-3 col-sm-4 col-6 py-3 inputesPaddingReport">
                                    <label for="memberType" class="form-label">Interest %</label>
                                    <input type="text" name="interest"class="form-control formInputsReport"
                                        id="interest" placeholder="Interest" required>
                                </div>
                                <div
                    ...
                    <table class="table table-bordered">
                        <thead class="table_head verticleAlignCenterReport">
                            <tr>
                                ...
                                <th class="fw-bold">Intrest Payable</th>
...
@push('script')
    <script>
        function createTableRow(sr, account, name, balance, interest, payable, grandTotal) {
            var row = `<tr>
                    <td>${sr}</td>
                    <td>${account}</td>
                    <td>${name}</td>
        ...
        $(document).ready(function() {
            ...
            $(document).on('submit', '#formData', function(event) {
                ...
                $.ajax({
                    url: '{{ route('ContributionList.getData') }}',
                    type: 'get',
                        ...
                        if (response['status'] == true) {
                            ...
                            for (var i = 0; i < data.length; i += entriesPerGroup) {
                                ...
                                groupData.forEach(function(item, index) {
                                    ...
                                    var payable = interest * balance/100;
                                    var grandTotal = payable + balance;
                                    groupTotal += parseFloat(grandTotal);

                                    var row = createTableRow(sr, account, name, balance, interest, payable, grandTotal);
                                    tableBody.append(row);
                                });

                                var groupGrandTotalRow = `<tr class="bg-success">
                                        <td colspan="6" class="text-white">Page ${i / entriesPerGroup + 1} </td>
