        ...
        <div class="col-12 ">
            <div class="card page_headings cards">
                <div class="card-body py-2">
                    <h4 class="py-2"><span class="text-muted fw-light">Masters / Inventory Module / </span> Create Tax</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-12 cards">
            <div class="card">
                <div class="card-body py-3">
                    <h5 class="card-action-title">Tax Master</h5>
...
<div class="modal fade" id="basicModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="taxMaster" action="{{route('masterupdate')}}" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="actiontype" value="taxMaster" />
                ...
                <div class="modal-body">
                    <div class="row row-gap-2">
                        ...
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport ">
                            ...
                            <input type="name" name="cess" id="cess" class="form-control formInputsReport" required />
                        </div>
                    <!-- </div>
                    <div class="row mb-3"> -->
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport ">
                            <label for="igst" class="form-label">IGST</label>
                            <input type="text" name="igst" id="igst" class="form-control formInputsReport">
                        </div>
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport ">
                            <label for="cgst" class="form-label">CGST</label>
                            <input type="text" name="cgst" id="cgst" class="form-control formInputsReport">
                        </div>
                    <!-- </div>
                    <div class="row"> -->
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport ">
                            <label for="sgst" class="form-label">SGST</label>
                            <input type="text" name="sgst" id="sgst" class="form-control formInputsReport">
                        </div>
...
$(document).ready(function() {
    var url = "{{url('statement/fetch')}}/taxMaster";
    var onDraw = function() {
    ...
    var options = [{
        ...
        },
        {
            ...
            render: function(data, type, full, meta) {
                ...
                var menu =
                    `<div style="display: flex;justify-content: space-evenly; align-items: center;"> <a href="javascript:void(0);" onclick="editTaxSetup('` + full.id + `','` +
                    full.name + `','` + full.cess + `','` + full.igst + `','` + full.cgst + `','` + full
                    .sgst + `','` + full.status + `')"><i class='fa-solid fa-pen-to-square border-0 iconsColorCustom'></i></a>`;
                menu += `<a onclick="deleteItem('` + full.id +
                    `', 'deleteTax')" href="javascript:void(0);" ><i class='fa-solid fa-trash iconsColorCustom'></i></a > </div>`;
                return menu;
            }
        }
    ];
...
function editTaxSetup(id, name, cess, igst, cgst, sgst, status) {
    $('#basicModal').find('.msg').text("Edit Tax Master");
    $('#basicModal').find('input[name="id"]').val(id);
    $('#basicModal').find('input[name="name"]').val(name);
    $('#basicModal').find('input[name="cess"]').val(cess);
    $('#basicModal').find('input[name="igst"]').val(igst);
    $('#basicModal').find('input[name="cgst"]').val(cgst);
    $('#basicModal').find('input[name="sgst"]').val(sgst);
