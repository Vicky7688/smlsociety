@extends('layouts.app')
@section('title', " Tehsil")
@section('pagetitle', "Tehsil")
        ...
        <div class="col-12 ">
            <div class="card page_headings cards">
                <div class="card-body py-2">
                    <h4 class="py-2"><span class="text-muted fw-light">Masters / Address Module / </span> Tehsil</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-12 cards">
            <div class="card">
                <div class="card-body py-3">
                    <h5 class="card-action-title">Tehsil</h5>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1"> <span class="msg">Add</span> Tehsil </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="stateMaster" action="{{route('masterupdate')}}" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="actiontype" value="tehsil" />
                ...
                <div class="modal-body">
                    <div class="row row-gap-2">
                        ...
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport ecommerce-select2-dropdown">
                            ...
                            <select name="districtId" id="status-org" class="select21 form-select formInputsSelectReport" data-placeholder="Active">
                                <option value="">Select</option>
...
$(document).ready(function() {
    ...
    var url = "{{url('statement/fetch')}}/tehsilmaster";
    var onDraw = function() {
    ...
    var options = [{
        ...
        },
        {
            ...
            render: function(data, type, full, meta) {
                ...
                var menu =
                    `<div style="display: flex;justify-content: space-evenly; align-items: center;"> <a href="javascript:void(0);" onclick="editSetup('` + full.id + `','` + full.stateId + `','` + full.districtId + `','`+ full.name + `','` + full.status +
                    `')"><i class='fa-solid fa-pen-to-square border-0 iconsColorCustom'></i></a>`;
                menu += `<a onclick="deleteItem('` + full.id +
                    `', 'deleteTehsil')" href="javascript:void(0);" ><i class='fa-solid fa-trash iconsColorCustom'></i></a > </div>`;
...
function editSetup(id, state, dist, tehsilName, status) {
    $('#basicModal').find('.msg').text("Edit");
    $('#basicModal').find('input[name="id"]').val(id);

    // Assuming 'state' is the correct value for the option you want to select
    var stateSelect = $('#basicModal').find('select[name="stateId"]');
    stateSelect.val(state).trigger('change');

    // Assuming 'dist' is the correct value for the option you want to select
    var distSelect = $('#basicModal').find('select[name="districtId"]');
    distSelect.val(dist).trigger('change');

    $('#basicModal').find('input[name="name"]').val(tehsilName);
...
function capitalizeFirstName() {
    var nameInput = $('#tehsil [name="name"]');
