@extends('layouts.app')
@section('title', "Depot Master")
@section('pagetitle', "Depot Master")
        ...
        <div class="col-12 ">
            <div class="card page_headings cards">
                <div class="card-body py-2">
                    <h4 class="py-2"><span class="text-muted fw-light">Masters / Inventory Module / </span> Depot Master</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-12 cards">
            <div class="card">
                <div class="card-body py-3">
                    <h5 class="card-action-title">Depot Master</h5>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1"> Add Depot </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="depotMaster" action="{{route('masterupdate')}}" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="actiontype" value="depotMaster" />
                ...
                <div class="modal-body">
                    <div class="row row-gap-2">
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport">
                            ...
                            <input type="text" name="depotName" id="depotName" class="form-control formInputsReport" required />
                        </div>
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport">
                            <label for="salesmanName" class="form-label">Salesman</label>
                            <input type="name" name="salesmanName" id="salesmanName" class="form-control formInputsReport" required />
                        </div>
                    <!-- </div>
                    <div class="row mb-3"> -->
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport">
                            <label for="phone" class="form-label">Phone</label>
                            <input type="text" name="phone" id="phone" class="form-control formInputsReport" maxlength="10"
...
$(document).ready(function() {
    var url = "{{url('statement/fetch')}}/depotMaster";
    var onDraw = function() {
    ...
    var options = [{
        ...
        },
        {
            ...
            render: function(data, type, full, meta) {
                ...
                var menu =
                    `<div style="display: flex;justify-content: space-evenly; align-items: center;"> <a href="javascript:void(0);" onclick="editDepotSetup('` + full.id + `','` +
                    full.depotName + `','` + full.salesmanName + `','` + full.phone + `','` + full
                    .address + `','` + full.status + `')"><i class='fa-solid fa-pen-to-square border-0 iconsColorCustom'></i></a>`;
                menu += `<a onclick="deleteItem('` + full.id +
                    `', 'deleteDepot')" href="javascript:void(0);" ><i class='fa-solid fa-trash iconsColorCustom'></i></a > </div>`;
                return menu;
            }
        }
    ];
...
function editDepotSetup(id, depotName, salesmanName, phone, address, status) {
    $('#basicModal').find('.msg').text("Edit Tax Master");
    $('#basicModal').find('input[name="id"]').val(id);
    $('#basicModal').find('input[name="depotName"]').val(depotName);
    $('#basicModal').find('input[name="salesmanName"]').val(salesmanName);
