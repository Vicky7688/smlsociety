@extends('layouts.app')
@section('title', "Purchase Client Master")
@section('pagetitle', "Purchase Client Master")
        ...
        <div class="col-12 ">
            <div class="card page_headings cards">
                <div class="card-body py-2">
                    <h4 class="py-2"><span class="text-muted fw-light">Masters / Inventory Module / </span> Purchase Client</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-12 cards">
            <div class="card">
                <div class="card-body py-3">
                    <h5 class="card-action-title">Purchase Client Master</h5>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1"> Add Purchase Client </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="purchaseClientMaster" action="{{route('masterupdate')}}" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="actiontype" value="purchaseClientMaster" />
                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3"> -->
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport">
                            ...
                            <select name="state" id="status-org" class="select21 form-select formInputsSelectReport" data-placeholder="Active"
                        ...
                        <div class="col-lg-4 col-sm-4 col-6 py-2 inputesPaddingReport ecommerce-select2-dropdown">
                            <label for="status-org" class="form-label">District</label>
...
$(document).ready(function() {
    var url = "{{url('statement/fetch')}}/purchaseClientMaster";
    var onDraw = function() {
    ...
    var options = [{
        ...
        },
        {
            ...
            render: function(data, type, full, meta) {
                ...
                var menu =
                    `<div style="display: flex;justify-content: space-evenly; align-items: center;"> <a href="javascript:void(0);" onclick="editPurchaseClientSetup('` + full.id +
                    `','` +
                    full.name + `','` + full.city + `','` + full.state + `','` + full
                    .district + `','` + full.address + `','` + full.email + `','` +
                ...
                menu += `<a onclick="deleteItem('` + full.id +
                    `', 'deletePurchaseClient')" href="javascript:void(0);" ><i class='fa-solid fa-trash iconsColorCustom'></i></a > </div>`;
                return menu;
            }
        }
    ];
...
function editPurchaseClientSetup(id, name, city, state, district, address, email, phone, faxNo, gstNo, status) {
    $('#basicModal').find('.msg').text("Edit Purchase Client Master");
    $('#basicModal').find('input[name="id"]').val(id);
    $('#basicModal').find('input[name="name"]').val(name);
    $('#basicModal').find('input[name="city"]').val(city);
    var stateSelect = $('#basicModal').find('select[name="state"]');
    stateSelect.val(state).trigger('change');
    var districtSelect = $('#basicModal').find('select[name="district"]');
    ...
    console.log(id, name, city, state, district, address, email, phone, faxNo, gstNo, status);
}
