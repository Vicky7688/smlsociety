    ...
    <div class="row">
        <div class="col-12 ">
            <div class="card page_headings cards">
                ...
                <h4 class="py-2"><span class="text-muted fw-light">Masters / Address Module / </span> State</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-12 cards">
            <div class="card">
                <div class="card-body py-3">
                    <h5 class="card-action-title">State</h5>
...
<div class="modal fade" id="basicModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="state" action="{{route('masterupdate')}}" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="actiontype" value="state" />
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var url = "{{url('statement/fetch')}}/state";
        var onDraw = function() {
        ...
        var options = [{
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    var menu =
                        `<div style="display: flex;justify-content: space-evenly; align-items: center;"> <a href="javascript:void(0);" onclick="editStateSetup('` + full.id + `','` + full
                        .name + `','` + full.status + `')"><i class='fa-solid fa-pen-to-square border-0 iconsColorCustom'></i></a>`;
                    menu +=
                        `<a onclick="deleteItem('${full.id}', 'deleteState')" href="javascript:void(0);"><i class='fa-solid fa-trash iconsColorCustom'></i></a> </div>`;
                    return menu;
                }
            },
        ];
    ...
    function editStateSetup(id, name, status) {
        // Assuming '#basicModal' is your modal ID
        $('#basicModal').find('.msg').text("Edit State");
