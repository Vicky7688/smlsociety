        ...
        <div class="card-body py-2">
            <div class="row justify-content-between align-items-center">
                <div class="col-md-6 fdHeading">
                    <h4 class="py-2"><span class="text-muted fw-light">Transactions / </span>Fixed Deposit</h4>
                </div>
                <div class="col-md-3">
                                ...
                                <div class="tab-pane fade active show" id="fdDetails" role="tabpanel">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-2 col-md-3 col-sm-4 col-6 saving_column">
                                            ...
                                            <input type="text" id="accountNo" name="accountNo" class="form-control form-control-sm " placeholder="Account No"  autocomplete="off" />
                                             <div id="accountList" class="accountList"></div>
                                            <p class="error"></p>
                                        </div>
                                        <div class="col-lg-2 col-md-3 col-sm-4 col-6 d-none  saving_column">
                                            <label for="fdNo" class="form-label">Account No</label>
                                            <input type="text" id="fdNo" name="fdNo" class="form-control form-control-sm" placeholder="FD No"  readonly/>
                                            <div id="fdaccountList" class="fdaccountList"></div>
                                            <p class="error"></p>
                                        </div>
                                        <div class="col-lg-2 col-md-3 col-sm-4 col-6">
                                            ...
                                            <select name="fdType" id="fdType" class="select21 form-select form-select-sm Select" data-placeholder="Active">
                                                ...
                                                <option value="SahakaritaBond">Sahakarita Bond</option>
                                                <option value="SpecialBachatYojnaBond">Special Bachat Yojna Bond</option>
                                            </select>
                                            <p class="error"></p>
                                        </div>
                                        <div class="col-lg-2 col-md-3 col-sm-4 col-6     ">
                                            <label for="openingDate" class="form-label">Opening Date</label>
                                            <input type="date" id="openingDate" name="openingDate" class="form-control form-control-sm" placeholder="Opening Date" value="{{ Session::get('currentdate') }}" max="{{ now()->format('Y-m-d') }}" />
                                            <p class="error"></p>
                                        </div>
                                        <div class="col-lg-2 col-md-3 col-sm-4 col-6     ">
                                        ...
                                        <div class="col-lg-2 col-md-3 col-sm-4 col-12 py-2 saving_column   savingColumnButton">
                                            <div class="d-flex h-100 justify-content-end text-end">
                                                ...
                                                <div class="modal-footer pt-3 fdCustomButtons">
                                                    <a href="{{ route('fd.index') }}" type="button" id="newFdButton" class=" btn btn-secondary waves-effect waves-light d-none dynamic-buttons">New
                                                        FD</a>
                ...
                <table class="table text-center table-bordered">
                    <thead class="table_head verticleAlignCenterReport">
                        <tr>
                            ...
                            <th scope="col">Membership Acc No</th>
                            <th scope="col">FD Acc No</th>
                ...
                <form action="javascript:void(0)" id="matureFormData" name="matureFormData">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-4 col-6 saving_column saving_acc_column">
                            ...
                            <input type="text" id="savingAccNo" name="savingAccNo" class="form-control form-control-sm" placeholder="Saving Account No" readonly />
                            <div id="savingaccountList" class="fdaccountList"></div>
                            <p class="error"></p>
                        </div>

                        <div class="col-lg-4 col-md-4 col-sm-4 col-6     ">
                            <label for="maturePrincipal" class="form-label">Principal Amount</label>
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="renewFormData" name="renewFormData">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-2 col-md-3 col-sm-4 col-6     ">
                            <label for="renewFdNo" class="form-label">FD Acc No</label>
...
// Function to fetch account list (called on memberType change or accountNo keyup)
function getAccountList() {
    var memberType = $('#memberType').val();
    var accountNo = $('#accountNo').val();
    $.ajax({
        url: "{{ route('fd.getData') }}",
        type: "GET",
        data: {
        ...
        success: function(response) {
            if (response['status'] == true) {
                ...
                $("#accountList").html('<li class="list-group-item memberlist">No Data Found</li>');
            }
        },
        error: function() {
...
// Event listeners for memberType and accountNo
$("#memberType").on('change', getAccountList);
$("#accountNo").on('keyup', getAccountList);

// Handle click event on memberlist items from accountList
$(document).on('click', '#accountList .memberlist', function() {
    var accountNo = $(this).text();
    var memberType = $('#memberType').val();
    $("#accountList").html("");
    displayTable(memberType, accountNo, null);
});

// Handle click event on fdmemberlist items from fdaccountList
$(document).on('click', '#fdaccountList .fdmemberlist', function() {
    var fdaccountNoo = $(this).text();
    var memberType = $('#memberType').val();
    var accountNo = $('#accountNo').val();
    $('#fdNo').val(fdaccountNoo);
    $("#fdaccountList").html("");
    $("#accountList").html("");
    displayTable(memberType, accountNo, fdaccountNoo);
});

// Function to display the table based on memberType, accountNo, and optional FD accountNo
function displayTable(memberType, accountNo, fdaccountNoo = null) {
    $.ajax({
        url: "{{ route('fd.fetchData') }}",
        type: "GET",
        data: {
            memberType: memberType,
            accountNo: accountNo,
            fdaccountNoo: fdaccountNoo
        },
        dataType: 'json',
        success: function(response) {
            if (response['status'] == true) {
                if (!fdaccountNoo) {
                    $("#fdaccountList").html(response.fd_accounts);
                }
                // Populate the member name
                $('#memberName').html(response.member.name);
                // Populate the FD table
                ...
                $.each(fdRow, function(index, fd) {
                    var mdate = fd.status === "Matured" ? formatDate(fd.onmaturityDate) : formatDate(fd.maturityDate);
                    var iamount = fd.status === "Matured" ? fd.actualInterestAmount : fd.interestAmount;
                    var row = "<tr>" +
                        "<td>" + (sr--) + "</td>" +
                        "<td>" + fd.accountNo + "</td>" +
                        "<td>" + fd.fdNo + "</td>" +
                        ...
                        "<td>" + iamount + "</td>" +
                        "<td>" + mdate + "</td>" +
                        "<td>" + (parseInt(fd.principalAmount) + parseInt(iamount)) + "</td>" +
                        "<td>" + fd.status + "</td>" +
                ...
                $("#accountList").html(""); // Clear the account list after the table is displayed
            }
        }
    });
}
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response['status'] == true) {
                        ...
                        $('#printFdButton').removeClass('d-none').attr('printId', response.data.id);
