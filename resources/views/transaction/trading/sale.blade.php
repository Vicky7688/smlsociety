...
<div class="container-xxl flex-grow-1 py-3">
    <div class="row">
        <div class="col-md-12">
            <h4><span class="text-muted fw-light">Transactions / </span>Sale</h4>
        </div>
    </div>
    <div class="row">
                ...
                <div class="card">
                    <div class="card-body px-3 pt-2 pb-0">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2 col-12">
                                <label for="saleClient" class="form-label">Sale Client</label>
                                <select class="form-select" id="saleClient" name="saleClient">
                                    @if($saleClients)
                                        @foreach($saleClients as $saleClient)
                                            <option value="{{ $saleClient->id }}">{{ $saleClient->name }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="col-md-2 col-12">
                                <label for="depot" class="form-label">Sale Depot</label>
                            ...
                            <div class="col-md-2 col-12">
                                <label for="itemName" class="form-label">Name <span class="text-danger" id="itemStock"></span></label>
                        ...
                        <table class="table text-center table-bordered">
                            ...
                            <tbody id="tableBody">
                                @if(!empty($saleInvoice))
                                    <tr>
                                        <td>{{ $saleInvoice->invoiceNo }}</td>
                                        <td>{{ $saleInvoice->invoiceDate }}</td>
                                        <td>{{ $saleInvoice->client->name }}</td>
                                        <td>{{ $saleInvoice->saleDepot->depotName }}</td>
                                        <td>{{ $saleInvoice->grandTotal }}</td>
                                        <td>{{ $saleInvoice->paymentTypee->name }}</td>
                                        <td><button type="button" viewId="{{ $saleInvoice->id }}"
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response['status'] == true) {
                        ...
                        var itemStock = ' ('+response.itemStock+' '+itemData.unit+' left)';
                        $('#itemCode').val(itemData.code);
                        $('#itemName').val(itemData.name);
                        $('#itemUnit').val(itemData.unit);
                        $('#itemPrice').val(itemData.saleRate);
                        $('#itemStock').html(itemStock);
                        $('#itemQuantity').attr('max', response.itemStock);
                        $("#itemList").html("");
                        calculateItemTotal();
                    }
                }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response['status'] == true) {
                        ...
                        $('#saleClient').val(response.data.saleClient);
                        ...
                        localStorage.removeItem('items');
                        var itemsData = response.data.sale_detail;
